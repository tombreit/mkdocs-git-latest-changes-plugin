...
class GitLatestChangesPlugin(BasePlugin[GitLatestChangesPluginConfig]):
    ...
    def on_config(self, config: MkDocsConfig) -> MkDocsConfig:
        ...
        if self.config.link_to_generated_page and (
            not self.config.limit_to_docs_dir or not config.site_url
        ):
            raise PluginError(
    ...
    def on_files(
        self, files: Files, *, config: MkDocsConfig, **kwargs: Optional[Any]
    ) -> Union[Files, None]:
        """
